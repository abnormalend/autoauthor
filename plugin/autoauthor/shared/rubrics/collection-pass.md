...
were given):
- `binding.md` — what makes these works one book, the slate, the running
  order
...
- binding_delivery [cap 6] — Read `binding.md`'s statement of what unifies these works. Then read the works and ask whether it is DELIVERED or merely declared. Test: name the two works where the binding is most and least present. If the least is a work that would sit unchanged in a different collection, score 6 max. A binding that is only a shared setting is the commonest thin case: setting is where a collection happens, not what it is about.
