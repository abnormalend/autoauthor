...
terminal, substitute the path to `plugin/autoauthor` in this repo.)
...
`review` check them against the manuscript.
...
`characters.md`, `outline.md`, or `ch_NN`). `seed` renders each
...
What `seed` reads to generate ten concepts. Four parts, in this
order:
