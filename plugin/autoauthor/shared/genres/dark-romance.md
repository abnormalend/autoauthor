...
lead's chain changes. `draft` adds a row whenever a chapter moves the
balance in a way the ledger does not have, and corrects the last two columns
when a chapter lands differently from the plan. `revise` and
`review` re-verify the whole table against the manuscript, and are the
