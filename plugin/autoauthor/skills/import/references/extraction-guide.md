...
One section per artifact import produces. Follow the matching
section when extracting or writing that artifact. These mirror the
structures in `foundation/references/layer-guides.md`, but the
...
- `genre`, `genre_secondary`, `genre_modifiers`: the confirmed inference
  ...
  `general`.
- Leave `iteration`, `foundation_score`, `pillar_score`, `work_score`,
