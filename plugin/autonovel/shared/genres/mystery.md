...
**Which phase fills it.** `novel-seed` creates the file from this
template when it scaffolds the project. `novel-foundation` fills it,
...
solvability curve. `novel-draft` adds a row whenever a chapter plants a
clue the ledger does not have, and corrects the `Planted` column when a
chapter lands a clue somewhere other than planned. `novel-revise` and
`novel-review` re-verify the whole table against the manuscript.
