---
name: novel-import
description: Use when the user wants to import an existing story, manuscript, or draft written elsewhere into the autonovel pipeline — to revise it, continue it, or salvage its foundation for a re-draft.
---

# Novel Import — Existing Story → Pipeline Project

Reverse-engineers an existing manuscript into a full autonovel project:
...
1. **Gather inputs.** Ask for (a) the source — a file, several files,
   ...
   default and safety rules as novel-seed: suggest `~/novels/<tag>`,
...
8. **Commit and hand off.** `git add -A && git commit -m "import:
   ...
   `/autonovel:novel-foundation`, which runs the normal gated loop and
