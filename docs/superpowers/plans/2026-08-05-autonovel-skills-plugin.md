...
- 2026-08-05, Addendum (post-plan): built the novel-import skill (revise/continue/salvage modes) per the spec's future-skills sketch and user decisions (always-gated foundation, inferred+confirmed MYSTERY, propose-and-confirm splitting). novel-foundation's Exit is now state-keyed (revision vs drafting) with a TO-BE-OUTLINED completion guard; its fill pass fills marked outline remainders. Plugin now has 8 skills.
- 2026-08-05, Tasks 18–19: install done non-interactively via `claude plugin marketplace add` + `claude plugin install` CLI (no interactive session needed); marketplace description added to silence the one validation warning. Smoke test passed end to end: novel-seed built /tmp/novel-smoke (auto-picked original concept "The Ledger of Drowned Bells"), novel-foundation filled all seven layers and ran one judged iteration (7.0/7.0, kept, correctly below the 7.5 gate), zero first-novel leakage, scripts ran from the install path (slop scorer caught 2 tier-1 words in generated world.md). Smoke-friction fixes landed post-review: printf for the TSV header; fence-wrapped JSON counts as valid; TSV score column = overall_score with lore in description; canon must never cite MYSTERY.md (veiled duplication into visible layers instead). Cache refresh required uninstall+reinstall (marketplace update alone didn't recopy).
