...
6. **Provenance-agnostic phases:** each phase skill depends only on the layer
   docs present in the project and `state.json` — never on how they were
   produced. `novel-draft` must not assume `seed.txt` exists or that
   foundation was score-gated; `novel-revise`/`novel-review` must not assume
   chapters came from `novel-draft`. This keeps the door open for a future
   `novel-import` skill (see Future skills) and for hand-authored projects.
   The router treats `seed.txt` as optional.
...
## Future skills (out of scope for v1)

> **Update 2026-08-05:** `novel-import` was built as a post-v1 addendum.
> Design decisions: all imports run the full gated foundation loop;
> MYSTERY.md is inferred with an IMPORTED banner and confirmed by the
> author; chapter splitting is headings → patterns → propose-and-confirm.
> novel-foundation's Exit became state-keyed (all outlined chapters exist
> → revision, else drafting) with a TO-BE-OUTLINED completion guard.

- **`novel-import`** — take an existing story generated by other means and
  reverse-engineer it into a pipeline project: extract world.md,
  characters.md, outline.md, canon.md from the prose; run
  `voice_fingerprint.py` plus judge analysis to fill voice.md Part 2; split
  prose into `chapters/`; write a truthful `state.json`. Three entry modes:
  (a) revise a finished manuscript (enter at revision/review), (b) continue
  an unfinished story (enter at drafting mid-sequence), (c) salvage the
  foundation and re-draft (enter at foundation or drafting ch 1). Enabled by
  invariant 6; requires no changes to the phase skills. Open questions
  deferred to its own design: inferring MYSTERY.md from finished prose,
  whether extracted foundations must pass the foundation gate, chapter
  boundary detection for unstructured input.
